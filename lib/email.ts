...
export async function sendEmail(to: string, subject: string, text: string, link?: string): Promise<SendResult> {
  ...
  const from = process.env.EMAIL_FROM || "תיבת הדילמות <onboarding@resend.dev>";
  const html = `<div dir="rtl" style="font-family:Arial,sans-serif;font-size:15px;line-height:1.6;color:#1c2230">
    <p>${escapeHtml(text).replace(/\n/g, "<br>")}</p>
    ${link ? `<p><a href="${link}" style="display:inline-block;background:#d9601a;color:#fff;padding:10px 18px;border-radius:8px;text-decoration:none">פתיחה בתיבה</a></p>` : ""}
    <p style="color:#6f6b63;font-size:12px">הודעה אוטומטית מתיבת הדילמות.</p></div>`;
  if (!key) {
